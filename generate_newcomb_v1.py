	...
	while i < len(waitlist):
		...
		for p in p_same_date:			
			...
			for that_mg_string in existing_mg_symbols:
				...
				if len(this_mg_set) > len(that_mg_set):
					continue

				# print ("Debug 5.1, issubset: ", this_mg_set.issubset(that_mg_set), " this_mg_string: ", this_mg_string, " that_mg_string: ", that_mg_string)
				if this_mg_set.issubset(that_mg_set):
					is_newcomb = False
					break
		...
		sleep(0.1)
		pbar.update(len(batch_mg_strings))
...
print ("Debug 4, the length of new combinations: ", len(new_combinations))

# outliers_output_file_name = args.outputfile + "outliers_" + str(args.startyear) + "_" + str(args.endyear) + ".csv"
newcomb_output_file_name = args.outputfile + "newcomb_" + str(args.startyear) + "_" + str(args.endyear) + ".csv"


# for debug purpose, write the outliers to a file
with open(newcomb_output_file_name, 'w') as output_csv:
	writer = csv.writer(output_csv)
	# write header row
	writer.writerow(["appNo", "appDate", "mg_string", "nationality"])

	for p in new_combinations:
		# write to a file
